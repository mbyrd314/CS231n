...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    p = np.exp(scores[np.arange(N), y]) / np.sum(np.exp(scores), axis=1)
    ...
    #############################################################################
    #Gradient of the scores is the gradient of the softmax loss function
    dscores = (np.exp(scores).T / np.sum(np.exp(scores), axis=1)).T
    ys = np.zeros_like(dscores)
    ys[np.arange(N), y] = 1
    dscores -= ys
    dscores /= N
    
    #This can be determined by observing the dimensions of the arrays involved.
    dlayer2 = dscores.dot(W2.T)
    
    
    #This can also be determined by comparing the dimensions
    dW2 = layer2.T.dot(dscores)
    
    #Sum of the gradients of the loss function
    db2 = np.sum(dscores, axis=0)
    
    #The gradient of the ReLU is 1 for positive numbers and 0 otherwise.
    dlayer1 = dlayer2 * (layer1 > 0)
    
    #This is similar to the gradient for W2
    dW1 = X.T.dot(dlayer1)
    
    #This is similar to the gradient for b2
